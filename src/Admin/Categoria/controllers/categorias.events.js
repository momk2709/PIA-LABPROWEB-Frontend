...
const asginarCategoriaModal = document.getElementById(
  "asignar-categoria-modal"
);
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  categoriasTable.addEventListener("click", async (e) => {
    ...
    if (e.target.classList.contains("btn-primary")) {
      ...
      const categoria = response.categories.find(
    ...
    if (e.target.classList.contains("btn-warning")) {
      ...
      const asignarCategoriaId = document.getElementById(
        "asignar-categoria-id"
      );

      asignarCategoriaId.value = id;

      $("#asignar-categoria-modal").modal("show");
    }
  });
});
...
editarCategoriaForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    Swal.fire(
      "¡Actualizado!",
      `El curso ${response.category.id} se actualizo correctamente`,
      "success"
    ).then(() => {
...
crearCategoriaForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    Swal.fire(
      "¡Creado!",
      `El curso ${response.category.id} se creo correctamente`,
      "success"
    ).then(() => {
      // Aqui se cierra el modal
...
// Asignar categoria a un curso

const asignarCategoriaForm = document.getElementById("asignar-categoria-form");

asignarCategoriaForm.addEventListener("submit", async (e) => {
  e.preventDefault();

  const categoria_id = Number(
    asignarCategoriaForm["asignar-categoria-id"].value
  );
  ...
  try {
    ...
    Swal.fire({
      title: "¡Asignado!",
      text: `La categoria ${response.cursoCategory.categoria_id} se asigno correctamente al curso ${response.cursoCategory.curso_id}`,
    }).then(() => {
      // Aqui se cierra el modal
      $("#asignar-categoria-modal").modal("hide");
      $("#asignar-categoria-form").trigger("reset");
